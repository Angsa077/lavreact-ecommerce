...
import SubCategoryDetailsModal from '../../partials/modals/SubCategoryDetailsModal';
...
const SubCategoryList = () => {
    ...
    const [detailsSubCategory, setDetailsSubCategory] = useState(null);
    const [isLoading, setIsLoading] = useState(false);
    const [errors, setErrors] = useState([]);

    const getSubCategories = (pageNumber = 1) => {
        setIsLoading(true);
        axiosClient.get(`/sub-category?page=${pageNumber}&search=${input.search}&order_by=${input.order_by}&direction=${input.direction}&per_page=${input.per_page}`)
            .then(res => {
    ...
    const handleSubCategoryDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                setIsLoading(true)
                axiosClient.delete(`/sub-category/${id}`)
                    .then(res => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
        ...
        <>
            ...
            <div className='border border-slate-100 shadow-md mt-3'>
                <div className='flex justify-between items-center border py-1 bg-gray-50'>
                    <h2 className='ml-3 text-lg font-semibold text-blue-500 py-2'>Sub Category</h2>
                                    ...
                                    {subCategories.length === 0 ? (
                                        ...
                                        subCategories.map((subCategory, index) => (
                                            <tr key={index} className='text-xs sm:text-sm bg-white border-b  hover:bg-gray-50'>
                                                ...
                                                <td scope="col" className="px-2 py-1 sm:px-3 sm:py-2">
                                                    <p className='text-blue-500'>Serial : {subCategory.serial}</p>
                                                    <p className={`${subCategory.status == 'Active' ? 'text-green-500' : 'text-red-500'}`}>Status : {subCategory.status}</p>
                                                </td>
                                                <td scope="col" className="px-2 py-1 sm:px-3 sm:py-2">
                                                ...
                                                <td scope="col" className="text-center">
                                                    <div className="flex flex-col sm:flex-row items-center justify-center">
                                                        ...
                                                        <Link to={`/sub-category/edit/${subCategory.id}`}>
