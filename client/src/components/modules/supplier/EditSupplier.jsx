...
const EditSupplier = () => {
    ...
    const getSuppliers = () => {
        setIsLoading(true);
        axiosClient.get(`/supplier/${params.id}`)
            .then(res => {
    ...
    const handleSupplierUpdate = async (e) => {
        ...
        e.preventDefault();
        try {
            await axiosClient.put(`/supplier/${params.id}`, input);
            setIsLoading(false);
            navigate('/supplier')
            Toast.fire({
                icon: 'success',
    ...
    return (
        <>
            <BreadCrumb title={`Edit Supplier - ${input.name}`} />
            <div className='border border-slate-100 shadow-md mt-3'>
                <div className='flex justify-between border py-1'>
                    <h2 className='ml-3 text-lg font-semibold text-blue-500 py-2'>Edit Supplier</h2>
                    <button
                ...
                <form onSubmit={handleSupplierUpdate}>
                            ...
                            <div className="">
                                ...
                                <div className='flex'>
                                    ...
                                    <div className='mx-auto'>
                                        {input.logo && (
                                            <img
                                                src={input.logo instanceof File ? window.URL.createObjectURL(input.logo) : input.logo}
                                                alt="logo"
                    ...
                    <div className='px-8 mb-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-4 lg:grid-cols-8'>
                        ...
                        {!isLoading &&
                            <button className=" bg-blue-500 text-white p-1 px-3 font-bold text-md rounded-md hover:scale-105" type="submit">Update</button>
                        }
                    </div>
                </form>
            </div >
