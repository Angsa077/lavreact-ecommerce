...
const BrandList = () => {
    ...
    const getBrands = (pageNumber = 1) => {
        setIsLoading(true);
        axiosClient.get(`/brand?page=${pageNumber}&search=${input.search}&order_by=${input.order_by}&direction=${input.direction}&per_page=${input.per_page}`)
            .then(res => {
            ...
            <div className='border border-slate-100 shadow-md mt-3'>
                ...
                <div className='block sm:grid grid-cols-4 gap-4 mt-3'>
                    ...
                    <div className='grid grid-cols-1 mt-3 sm:mt-5 mx-5 hover:scale-105'>
                        <button type='button' className='py-1 text-sm bg-blue-500 text-white rounded-md border border-blue-500 shadow-md font-bold' onClick={() => getBrands(1)}>Search</button>
                    </div>
                </div>
                                    ...
                                    {brands.length === 0 ? (
                                        ...
                                        brands.map((brand, index) => (
                                            <tr key={index} className='text-xs sm:text-sm bg-white border-b  hover:bg-gray-50'>
                                                ...
                                                <td scope="col" className="px-2 py-1 sm:px-3 sm:py-2">
                                                    <p className='text-blue-500'>Serial : {brand.serial}</p>
                                                    <p className={`${brand.status == 'Active' ? 'text-green-500' : 'text-red-500'}`}>Status : {brand.status}</p>
                                                </td>
                                                <td scope="col" className="px-2 py-1 sm:px-3 sm:py-2">
