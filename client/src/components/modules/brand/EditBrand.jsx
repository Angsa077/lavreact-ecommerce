...
const EditBrand = () => {
    ...
    const getBrand = () => {
        setIsLoading(true);
        axiosClient.get(`/brand/${params.id}`)
            .then(res => {
    ...
    const handleBrandUpdate = async (e) => {
        ...
        e.preventDefault();
        try {
            await axiosClient.put(`/brand/${params.id}`, input);
            setIsLoading(false);
            navigate('/brand')
            Toast.fire({
                icon: 'success',
    ...
    return (
        <>
            <BreadCrumb title={`Edit Brand - ${input.name}`} />
            <div className='border border-slate-100 shadow-md mt-3'>
                <div className='flex justify-between border py-1'>
                    <h2 className='ml-3 text-lg font-semibold text-blue-500 py-2'>Edit Brand</h2>
                    <button
